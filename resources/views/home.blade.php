...
<script>
    ...
    @foreach($latestNews as $news)
        <article class="group flex flex-col md:flex-row gap-6 border-b border-slate-100 pb-8 last:border-0">
            <a href="{{ route('news.show', $news->slug) }}" class="w-full md:w-1/3 aspect-video rounded-2xl overflow-hidden bg-slate-100 relative shrink-0 shadow-sm group-hover:shadow-md transition-shadow">
                @if($news->pin_order)
