...
<header id="main-header" class="bg-white border-b border-gray-100 py-4 relative z-50 transition-all duration-300">
    ...
    <div class="container mx-auto px-4 lg:px-8 flex justify-between items-center gap-4">
        ...
        <a href="/" class="flex items-center gap-3 group select-none shrink-0 animate-fade-in-up bg-transparent" 
...
<script>
    ...
    @foreach($latestNews as $news)
        <article class="group flex flex-col md:flex-row gap-6 border-b border-slate-100 pb-8 last:border-0">
            <a href="{{ route('news.show', $news->slug) }}" class="w-full md:w-1/3 aspect-video md:aspect-[4/3] overflow-hidden bg-slate-200 relative shrink-0 shadow-sm group-hover:shadow-md transition-all">
                @if($news->pin_order)
                    <div class="absolute top-2 left-2 bg-brand-red text-white text-[10px] font-bold px-2 py-1 z-10">HEADLINE</div>
                @endif
                
                <img src="{{ Storage::url($news->thumbnail) }}" alt="{{ $news->title }}" class="w-full h-full object-contain transition-transform duration-700 group-hover:scale-105">
            </a>
