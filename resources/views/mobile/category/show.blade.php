...
<body class="bg-white text-slate-800 flex flex-col min-h-screen" x-data="{ mobileMenu: false, searchOpen: false }">
    ...
    <div class="bg-white border-b border-slate-100 overflow-x-auto no-scrollbar sticky top-0 z-40">
        <div class="flex items-center px-4 h-12 gap-2 min-w-max">
            ...
            {{-- Loop Kategori (Menggunakan Logika Aman: $navCat) --}}
            @foreach($categories as $navCat)
                @php
                    // Logika: Cek apakah user sedang di Halaman Kategori ini?
                    // (Karena ini file category/show.blade.php, request()->url() akan cocok)
    ...
    {{-- SIDEBAR MOBILE MENU (Slide dari Kanan) --}}
        ...
        <div class="space-y-2">
            ...
            @foreach($categories as $navCat)
                ...
                <a href="{{ route('category.show', $navCat->slug) }}" 
                   ...
                   {{ $isActive ? 'text-brand-red bg-red-50 font-bold' : 'text-slate-700 hover:bg-slate-50' }}">
                    {{ $navCat->name }}
                </a>
            @endforeach
        </div>
    ...
    <main class="flex-grow bg-white">
        <div class="px-4 py-4">
            <div class="mb-6 pb-2 border-b border-slate-100">
                ...
                {{-- Gunakan variabel $category utama halaman --}}
