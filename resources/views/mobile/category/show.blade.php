...
<head>
    ...
    <title>Kategori {{ $category->name }} - Gaung Nusra</title>
...
<body class="bg-white text-slate-800 flex flex-col min-h-screen" x-data="{ mobileMenu: false, searchOpen: false }">
    ...
    {{-- 3. NAVBAR KATEGORI (UNIVERSAL LOGIC - SAFE) --}}
    <nav class="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-100 shadow-sm transition-all">
        <div class="px-4 py-2">
            <div class="flex items-center gap-2 h-10 overflow-x-auto no-scrollbar">
                ...
                <a href="/" 
                   class="shrink-0 px-4 py-1.5 rounded-full text-xs font-medium border whitespace-nowrap transition-colors
                   {{ request()->is('/') ? 'bg-brand-red text-white border-brand-red font-bold' : 'bg-transparent text-slate-600 border-transparent hover:bg-slate-50' }}">
                    Berita Utama
                </a>
                ...
                @foreach($categories as $navCat)
                    @php
                        // Logika Aman:
                        // 1. Cek Halaman Kategori
                        $isCategoryPage = request()->url() == route('category.show', $navCat->slug);
                        
                        // 2. Cek Halaman Berita (Pastikan route-nya 'news.show' dulu, baru cek $news)
                        //    Ini PENTING agar tidak error "Undefined property categories" di halaman ini
                    ...
                    <a href="{{ route('category.show', $navCat->slug) }}" 
                       ...
                       {{ $isActive ? 'bg-brand-red text-white border-brand-red font-bold' : 'bg-transparent text-slate-600 border-transparent hover:bg-slate-50' }}">
                        {{ $navCat->name }}
                    </a>
                @endforeach
    ...
    {{-- 4. SIDEBAR MOBILE MENU (SLIDE FROM RIGHT) --}}
    ...
    <div x-show="mobileMenu" class="fixed top-0 right-0 h-full w-[80%] max-w-[300px] bg-white z-[70] shadow-2xl p-6 overflow-y-auto" 
        ...
        <div class="space-y-2">
            ...
            @foreach($categories as $navCat)
                {{-- Gunakan logika yg sama dengan Navbar agar konsisten --}}
                @php
                    $isCategoryPage = request()->url() == route('category.show', $navCat->slug);
                    $isActive = $isCategoryPage; // Di menu burger, cukup cek halaman saja
                @endphp
                <a href="{{ route('category.show', $navCat->slug) }}" 
            ...
            <div class="flex flex-col gap-4" id="news-container">
                ...
                @else
                    <div class="text-center py-12 px-4 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                        ...
                        <p class="text-xs text-slate-500 font-medium">Belum ada berita di kategori ini.</p>
                    </div>
                @endif
            </div>
   ...
   <footer class="bg-brand-misty pt-10 pb-8 border-t border-slate-200 mt-8">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // Load More Logic (Category Specific)
            $('#btn-load-more').click(function() {
                ...
                // PENTING: Gunakan window.location.href agar tetap di URL kategori saat pagination
